...
public class GuideCopy1_5 : GuideBasic {
	...
	private static GuideCopy1_5 _instance;
	public static GuideCopy1_5 Instance
	{
		get
		{
			if(!_instance)
			{
				_instance = GameObject.FindObjectOfType(typeof(GuideCopy1_5)) as GuideCopy1_5;
				if(!_instance)
				{
					GameObject gm = new GameObject("GuideCopy1_5");
					_instance = gm.AddComponent(typeof(GuideCopy1_5)) as GuideCopy1_5;
	...
	IEnumerator ShowGuideLeader(GUIDE_COPY_STEP currentstep){
		...
		case GUIDE_COPY_STEP.END:
			...
			RecoverKeyObject();
			GuideManager.Instance.FinishedStep(GuideManager.GUIDE_STEP.COPY1_5);
