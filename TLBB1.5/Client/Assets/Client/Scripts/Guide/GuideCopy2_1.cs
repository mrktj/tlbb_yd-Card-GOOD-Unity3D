...
public class GuideCopy2_1 : GuideBasic {
	...
	private static GuideCopy2_1 _instance;
	public static GuideCopy2_1 Instance
	{
		get
		{
			if(!_instance)
			{
				_instance = GameObject.FindObjectOfType(typeof(GuideCopy2_1)) as GuideCopy2_1;
				if(!_instance)
				{
					GameObject gm = new GameObject("GuideCopy2_1");
					_instance = gm.AddComponent(typeof(GuideCopy2_1)) as GuideCopy2_1;
	...
	IEnumerator ShowGuideLeader(GUIDE_COPY_STEP currentstep){
		...
		case GUIDE_COPY_STEP.END:
			...
			RecoverKeyObject();
			GuideManager.Instance.FinishedStep(GuideManager.GUIDE_STEP.COPY2_1);
