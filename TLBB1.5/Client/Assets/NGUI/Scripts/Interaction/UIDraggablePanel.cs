...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	public delegate void OnDragFinished (DragonMode dragonMode);

    private DragonMode curDragonMode = DragonMode.None;
    ...
    public bool restrictByRealBounds = false;
	/// <summary>
    ...
    //增加一个变量记录Panel的初始坐标（拖拽返回时算的坐标不精确，这里调整）
    private Vector3 vecPosOrigen;

    //记录当前拖拽的方式
    private OnDragonMode vecOnDragonMode = OnDragonMode.None;

    //开始拖拽时的坐标
    private Vector2 startDragonPos;
	...
	private bool m_bHorizoning;
	private bool m_bBeingJudge;
	...
	void Awake ()
	{
        ...
        vecPosOrigen = new Vector3();
        vecPosOrigen.x = mPanel.transform.localPosition.x;
        vecPosOrigen.y = mPanel.transform.localPosition.y;
        vecPosOrigen.z = mPanel.transform.localPosition.z;
	}
	...
	public void Press (bool pressed)
	{
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			if (pressed)
			{
                ...
                startDragonPos = UICamera.currentTouch.pos;
			}
			else
			{
                Vector2 pos = UICamera.currentTouch.pos - startDragonPos;
				if(vecOnDragonMode == OnDragonMode.LeftOrRight)
				{
					if(pos.x >= 100)
					{
						curDragonMode = DragonMode.Right;
					}
                    else if (pos.x <= -100)
					{
						curDragonMode = DragonMode.Left;
					}
                    else
                    {
                        curDragonMode = DragonMode.None;
                    }
	...
	public void Drag ()
	{
		if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
				...
				if(curScale.x > 0 && curScale.y > 0 && !m_bBeingJudge && (offset.x != 0 || offset.y != 0))
				{
					m_bHorizoning = Mathf.Abs(offset.x) > Mathf.Abs(offset.y);
					m_bBeingJudge = true;
				}
				if(m_bBeingJudge)
				{
					if(m_bHorizoning)
					{
						curScale.x = scale.x;
						curScale.y = 0;
                        vecOnDragonMode = OnDragonMode.LeftOrRight;
					}
					else
					{
						curScale.y = scale.y;
						curScale.x = 0;
                        vecOnDragonMode = OnDragonMode.TopOrBottom;
					}
	...
	void LateUpdate ()
	{
		...
		if (mShouldMove && !mPressed)
		{
			...
			if (mMomentum.magnitude > 0.0001f)
			{
				...
				if (mMomentum.magnitude < 0.0001f && onDragFinished != null) onDragFinished(DragonMode.None);
				return;
			}
			else
			{
