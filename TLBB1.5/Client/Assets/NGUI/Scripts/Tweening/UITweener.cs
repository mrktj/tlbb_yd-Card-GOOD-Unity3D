...
public abstract class UITweener : IgnoreTimeScale
{
    ...
    //选择是否在pingPong模式下也调 回调函数
    public bool bPingPongStyleCallFun = false;
    //FactorTrans 改变的次数
    private int nFactorTrans = 0;
	...
	void Update ()
	{
		...
		else if (style == Style.PingPong)
		{
            ...
            //当pingpong了一次后（也就是mFactor 从 0-1,再从 1-0后调用）
            if (eventReceiver != null 
                     && !string.IsNullOrEmpty(callWhenFinished)
                        &&nFactorTrans >= 2 
                             && bPingPongStyleCallFun)
			{
				nFactorTrans = 0;
