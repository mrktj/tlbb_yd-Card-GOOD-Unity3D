...
void _chartBoostTrackEventWithMetadata( const char * eventIdentifier, const char * metadata )
{
	...
	NSError *error = nil;
	NSDictionary *dictionaryParsed = [NSJSONSerialization JSONObjectWithData:data options:0 error:&error];
	
	if( error )
	{
		NSLog( @"error parsing metadata: %@", error );
		return;
	}
	
	[[CBAnalytics sharedAnalytics] trackEvent:GetStringParam( eventIdentifier ) withMetadata:dictionaryParsed];
}
...
void _chartBoostTrackEventWithValueAndMetadata( const char * eventIdentifier, float value, const char * metadata )
{
	...
	[[CBAnalytics sharedAnalytics] trackEvent:GetStringParam( eventIdentifier ) withValue:[NSNumber numberWithFloat:value] andMetadata:dictionaryParsed];
}
