 ...
 */
public abstract class DownloaderService extends CustomIntentService implements
		IDownloaderService {
	...
	final private IStub mServiceStub = DownloaderServiceMarshaller
			.CreateStub(this);
		...
		if (db.mStatus == 0) {
			...
			if (null != infos) {
				for (DownloadInfo info : infos) {
					if (!Helpers.doesFileExist(context, info.mFileName,
							info.mTotalBytes, true)) {
	...
	private class CyouLVLRunnable implements Runnable {
		CyouLVLRunnable(Context context, PendingIntent intent,long fileLenthc,String filenamec) {
			Log.v(LOG_TAG,"cyouLVLRunnable");
			mContext = context;
			mPendingIntent = intent;
			fileLength = fileLenthc;
			filename = filenamec;
		}
		...
		public void run() {
			...
			try {
				...
				try {
					...
					switch (startDownloadServiceIfRequired(mContext,
							mPendingIntent, serviceClass)) {
	...
	public void updateLVL(final Context context) {
		...
		try {
				...
				if (urlcon.getResponseCode() >= 400) {
					Log.v(LOG_TAG, "game server error");
				}
				if (fileLength <= 0)
					Log.v(LOG_TAG, "can not get file size");

				Log.v(LOG_TAG, "file size:" + fileLength / 1024 + "K");

				String trueurl = urlcon.getURL().toString();
				filename = trueurl.substring(trueurl.lastIndexOf('/') + 1);
				Log.v(LOG_TAG, "filename");
				
				//h.post(new CyouLVLRunnable(c, mPendingIntent,fileLength,filename));
			} catch (MalformedURLException e) {
				e.printStackTrace();
				throw new RuntimeException("get resource file info error");
			} catch (IOException e) {
				e.printStackTrace();
				throw new RuntimeException("get resource file info error");
			}

			Log.v(LOG_TAG, "get resource file info ok");
	...
	protected void onHandleIntent(Intent intent) {
		...
		try {
			...
			if (null != pendingIntent) {
				...
				Log.e(LOG_TAG,
						"Downloader started in bad state without notification intent.");
				return;
			}
			
			pollNetworkState();
			
			if(false == this.mIsConnected)
			{
				mNotification.onDownloadStateChanged(IDownloaderClient.STATE_PAUSED_NETWORK_UNAVAILABLE);
			...
			for (DownloadInfo info : infos) {
				...
				if (info.mStatus == STATUS_SUCCESS) {
					...
					if (!Helpers.doesFileExist(this, info.mFileName,
							info.mTotalBytes, true)) {
						info.mStatus = 0;
	...
	public void onCreate() {
		super.onCreate();
		try {
			mPackageInfo = getPackageManager().getPackageInfo(getPackageName(),
					0);
			ApplicationInfo ai = getApplicationInfo();
			CharSequence applicationLabel = getPackageManager()
	...
	public String generateTempSaveFileName(String fileName) {
		String path = Helpers.getSaveFilePath(this) + File.separator + fileName
				+ TEMP_EXT;
		return path;
	}
	...
	public void notifyUpdateBytes(long totalBytesSoFar) {
		...
		if (0 != mMillisecondsAtSample) {
			...
			float currentSpeedSample = (float) bytesInSample
					/ (float) timePassed;
		...
		mNotification.onDownloadProgress(new DownloadProgressInfo(mTotalLength,
				totalBytesSoFar, timeRemaining, mAverageDownloadSpeed));
	...
	public void onClientUpdated(Messenger clientMessenger) {
		Log.e(LOG_TAG,"DonwloaderService onClientUpdated");
