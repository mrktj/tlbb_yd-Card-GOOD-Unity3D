        ...
        for k,v in mod_dict.items():
            if k == 'patches':
                ...
                for p in v:
                    ...
                    try:
                        expr = re.compile(re.escape(file_name))
                    except re.error:
                        expr = None
...
def test(argv=None):
    ...
    proj = XcodeProject.Load('/Users/chenbin/test/test.xcodeproj/project.pbxproj')
    proj.add_file('System/Library/Frameworks/SystemConfiguration.framework', tree='SDKROOT')
    proj.backup()
    proj.save()
